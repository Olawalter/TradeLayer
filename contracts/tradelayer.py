...
MAX_CARRIER_REF      = 64                # real booking references are far
                                         # shorter; the cap exists because this
                                         # string is printed into the prompt
...
# Adjudication is a live model call under consensus: it takes minutes, not
# seconds. Every path that leaves a trade needing one more round must leave at
# least this much room, or the trade dead-ends in `adjudicating` — and a
# dead-ended trade pays the buyer 100% at recovery, whatever the panel found.
MIN_ADJUDICATION_TIME = 600
...
class TradeLayer(gl.Contract):
    ...
    def create_trade(
        ...
        _require(res_window >= RESPONSE_WINDOW + app_window + MIN_ADJUDICATION_TIME,
                 ...
                 f"and at least {MIN_ADJUDICATION_TIME} seconds to adjudicate")
    ...
    def mark_shipped(self, trade_id: str, carrier_reference: str) -> None:
        ...
        _require(len(ref) <= MAX_CARRIER_REF,
                 "[EXPECTED] the carrier reference is too long")
    ...
    def mark_delivered(self, trade_id: str) -> None:
        ...
        now = self._utc_now()
        # Recording delivery starts EVERY clock that protects the buyer: the
        # dispute window, the resolution deadline and the recovery deadline all
        # hang off this instant. So the seller may not choose it freely.
        #
        # The buyer may record delivery whenever they like — they are the party
        # who knows when the goods actually arrived, and starting their own
        # clock early is their business. The seller may only record it from the
        # agreed delivery deadline onwards. Otherwise the seller marks delivery
        # in the same breath as shipment, the dispute window expires while the
        # goods are still at sea, and `close_undisputed` hands over the escrow
        # for a delivery nobody made.
    ...
    def submit_evidence(
        ...
        if tier == TIER_SUPPORTING:
            ...
            # A PARTY_CLAIM has no document, so it may carry no digest — but it
            # may never carry FREE TEXT. This field is printed into the panel's
            # prompt beside every evidence row, so anything accepted here is an
            # injection channel: multi-line text could forge an [AUTHORITATIVE]
            # row, the one tier no party is permitted to write.
        ...
        def adjudicate_case() -> str:
            ...
            for r in ev_rows:
                ev_lines.append(
                    ...
                    f"  sha256: {_sanitize(str(r['hash']))[:64] or '(none)'}\n"
            ...
            prompt = (
                ...
                f"  carrier: {agreement['carrier']} ref "
                f"{_sanitize(agreement['carrier_reference'])[:MAX_CARRIER_REF]}\n"
    ...
    def adjudicate(self, trade_id: str) -> None:
        ...
        if not bool(data.get("ok", False)):
            ...
            #
            # And the round is NOT charged. A round that decided nothing must
            # cost nothing: charging it means three transient model failures
            # exhaust the trade while its deadline is still open, the case can
            # never be decided, and the escrow falls to the buyer whatever the
            # merits. That is also something a party could aim for — evidence
            # is theirs to write before the freeze, and text that reliably
            # breaks a structured reply is not hard to write.
            t.status = STATUS_DISPUTED
            t.reason_code = str(data.get("error_class", "LLM_ERROR"))[:60]
            return

        # Charged only once the panel has actually produced a usable verdict.
        ...
        # The appeal window is CLAMPED so it can never outlive the point at
        # which a further round is still possible. Left unclamped, a verdict
        # reached late in the resolution window opens an appeal window that
        # closes after the resolution deadline: `finalize` refuses (the window
        # is open) and `adjudicate` refuses (the deadline has passed), and the
        # trade has no terminal state left. That is not a stalemate — it pays
        # the buyer the whole escrow at recovery, so the losing party is the
        # one who benefits from causing it.
        latest_appeal = int(t.resolution_deadline) - MIN_ADJUDICATION_TIME
        proposed_appeal = now + int(t.appeal_window)
        if proposed_appeal > latest_appeal:
            proposed_appeal = latest_appeal
        # If there is no room left at all the window is simply already closed,
        # which `finalize` reads correctly — it never blocks.
        t.appeal_deadline = u256(max(0, proposed_appeal))
    ...
    def submit_appeal(self, trade_id: str) -> None:
        ...
        # Not "is the deadline in the future" but "is there time to DO the
        # thing an appeal asks for". Granting an appeal that cannot be heard
        # strands the trade in `adjudicating`, and a stranded trade refunds the
        # buyer in full — so without this, appealing is a free option for
        # whichever party the verdict went against.
        _require(now + MIN_ADJUDICATION_TIME <= int(t.resolution_deadline),
                 "[EXPECTED] there is not enough time left before the resolution "
                 "deadline to hear an appeal")
    ...
    def claim_timeout_refund(self, trade_id: str) -> None:
        """
        Terminal escape, so funds cannot strand behind a trade that never
        resolved. No counterparty cooperation, no owner key.

        Recovery is an ANSWER TO SILENCE, not a way to win. Two rules follow
        from that, and both exist because without them the buyer is paid for
        running the clock out:

        1. If a verdict was already reached, recovery honours it. Overwriting a
           recorded SELLER_WIN with a full refund would make every adverse
           verdict appealable-by-attrition — wait out the clock and collect.
        2. A delivered trade nobody disputed is not "unresolved". Its dispute
           window closed with no claim, which is itself the answer, and
           `close_undisputed` is permissionless — anyone, including the buyer,
           can call it. Refusing here strands nothing.
        """
        t = self._trade(trade_id)
        _require(gl.message.sender_address.as_hex == t.buyer,
        ...
        undisputed = (t.status == STATUS_DELIVERED
                      and trade_id not in self.disputes
                      and now > int(t.dispute_deadline))
        _require(not undisputed,
                 "[EXPECTED] this delivered trade was never disputed — it settles to "
                 "the seller through close_undisputed, which anyone may call")

        held = int(t.deposited_amount)
        _require(held > 0, "[EXPECTED] this trade holds no escrow")

        if t.decision == "":
            # Nothing was ever decided: the buyer recovers the whole deposit.
            t.buyer_paid = u256(held)
            t.seller_paid = u256(0)
            t.decision = DECISION_BUYER_WIN
            t.payout_bps = u256(BPS_DENOMINATOR)
            buyer_amount, seller_amount = held, 0
        else:
            # A verdict exists. Settle it, at the payout the findings produced.
            buyer_amount, seller_amount = self._split(t)
            t.buyer_paid = u256(buyer_amount)
            t.seller_paid = u256(seller_amount)
